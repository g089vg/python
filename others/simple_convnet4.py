...
class CNN(Chain):
    def __init__(self):
        super(CNN, self).__init__(
            conv1  = L.Convolution2D(None,16, ksize=3, pad=1),
            conv3  = L.Convolution2D(None,32, ksize=3, pad=1),
            conv5  = L.Convolution2D(None,64, ksize=3, pad=1),
            conv7  = L.Convolution2D(None,128, ksize=3, pad=1),
            conv9  = L.Convolution2D(None,256, ksize=3, pad=1),
            conv11 = L.Convolution2D(None,512, ksize=3, pad=1),
            conv13 = L.Convolution2D(None,1024, ksize=3, pad=1),
            conv14 = L.Convolution2D(None,1024, ksize=3, pad=1),
            conv15 = L.Convolution2D(None,1024, ksize=3, pad=1),
...
#    trainer.extend(extensions.snapshot(filename='snapshot_epoch-{.updater.epoch}'))
    ...
    trainer.run()
    serializers.save_npz("CNN_model"+number+".npz", net) # npz形式で書き出し
    del trainer
